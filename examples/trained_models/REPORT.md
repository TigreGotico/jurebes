...
- **Default autoencoder + LogReg underperforms.** With the registry's
  default `(64, 16, 64)` bottleneck, `autoencoder_logreg` collapses on
  high-class-count datasets (0.27 on BANKING77, 0.24 on CLINC-150).
  The bottleneck is too aggressive — a search-tuned variant would
  recover ground, but that is not part of the default portfolio.
...
- The autoencoder portfolio entries use jurebes' default
  `(64, 16, 64)` bottleneck. Search-tuned autoencoders would land
  meaningfully higher — not investigated here because the goal was a
  comparison of *defaults*, not a parameter sweep.
